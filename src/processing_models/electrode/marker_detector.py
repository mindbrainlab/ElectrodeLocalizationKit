...
from .util import ColorEnhancementUtil
...
class MarkerDetector:
    def __init__(
        ...
        """
        Initialize green marker detector.
        """
    ...
    def _preprocess_image(self, image: np.ndarray) -> np.ndarray:
        """
        Enhance green regions and convert to a normalized grayscale for processing.
        """
        # Enhance green regions in HSV color space (overboost saturation and value)
        enhanced = ColorEnhancementUtil.enhance_green_in_hsv(image, 60.0, 25.0, 2.5, 2.5)

        # Convert to LAB and extract a-channel
        lab_image = cv2.cvtColor(enhanced, cv2.COLOR_RGB2LAB)
        _, a_channel, _ = cv2.split(lab_image)

        # Smooth the a-channel to reduce noise
        a_channel_smooth = cv2.GaussianBlur(a_channel.astype(np.float32), (3, 3), 1.0)

        # Convert to uint8 (for thresholding)
        a_uint8 = cv2.normalize(a_channel_smooth, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
        return a_uint8

    def _apply_morphological_operations(self, binary_mask: np.ndarray) -> np.ndarray:
        """
        Clean up binary mask using opening and closing operations.
        """
    ...
    def _filter_contours(
        self, morph_mask: np.ndarray, view_type: ViewType, use_frst: bool = False
    ) -> np.ndarray:
        """
        Filter contours by area and circularity, applying view-specific masks.
        """
        ...
        for contour in contours:
            ...
            if hasattr(view_type, "name"):
                x, y, w, h = cv2.boundingRect(contour)
                if view_type.name == "BACK" and y > 700:
                    continue
                if view_type.name == "BACK_TOP" and y > 850:
                    continue
                if view_type.name == "BACK_RIGHT" and y > 750 and x < 612:
                    continue

            area = cv2.contourArea(contour)
            min_area = self.min_area or view_type.marker_cfg.min_area or 750
            if area < min_area:
                continue
            ...
            threshold = 0.25 if use_frst else 0.3
    ...
    def _apply_dog_and_hough(
        ...
        """
        Detect circular markers using Difference of Gaussians + Hough Transform.
        """
        # Apply DoG filtering
        masked = cv2.bitwise_and(a_uint8, a_uint8, mask=filtered_mask)
        ...
        min_dist = self.min_distance or view_type.marker_cfg.min_distance or 75
        min_r = self.min_radius or view_type.marker_cfg.min_radius or 25
        max_r = self.max_radius or view_type.marker_cfg.max_radius or 40
    ...
    def detect_traditional(
        ...
        """
        Detect markers using thresholding + morphology + Hough transform.
        """
        a_uint8 = self._preprocess_image(image)

        fg_pixels = a_uint8[foreground_mask > 0]
        percentile_thresh = min(np.percentile(fg_pixels, 5), 128) if len(fg_pixels) > 0 else 128
        _, binary = cv2.threshold(a_uint8, percentile_thresh, 255, cv2.THRESH_BINARY_INV)
        binary = cv2.bitwise_and(binary, binary, mask=foreground_mask)

        morph_mask = self._apply_morphological_operations(binary)
        filtered_mask = self._filter_contours(morph_mask, view_type, use_frst=False)

        return self._apply_dog_and_hough(image, a_uint8, filtered_mask, view_type, use_frst=False)
    ...
    def detect_frst(
        ...
        """
        Detect markers using FRST (Fast Radial Symmetry Transform).
        """
        a_uint8 = self._preprocess_image(image)

        # Enhance circular structures
        tophat_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (25, 25))
        tophat = cv2.morphologyEx(a_uint8, cv2.MORPH_TOPHAT, tophat_kernel)
        frst_input = cv2.subtract(a_uint8, tophat)
        ...
        _, frst_binary = cv2.threshold(frst_normalized, 0, 255, cv2.THRESH_OTSU)
        ...
        filtered_mask = self._filter_contours(morph_mask, view_type, use_frst=True)

        return self._apply_dog_and_hough(image, a_uint8, filtered_mask, view_type, use_frst=True)
    ...
    def _combine_results(
        ...
        for x1, y1, r1, method1 in sorted(combined, key=lambda c: c[2], reverse=True):
            ...
            for i, (x2, y2, r2, method2) in enumerate(unique):
                ...
                min_dist = self.min_distance or view_type.marker_cfg.min_distance or 0
        ...
        if unique:
            return np.array([[(x, y, r) for x, y, r, _ in unique]], dtype=np.uint16)
