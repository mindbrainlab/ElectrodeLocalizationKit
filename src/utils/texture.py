...
def compute_hough_circles(
    ...
    min_radius: int,
    max_radius: int,
    rgb_circles_color: tuple[int, int, int] = HOUGH_CIRCLES_COLOR,
) -> tuple[np.ndarray, list[np.uint16] | None]:
...
def compute_difference_of_gaussians(
    image: np.ndarray, ksize: int, sigma: float, F: float, threshold_level: int
) -> np.ndarray:
